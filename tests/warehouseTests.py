...
class TestWareHouseAPI(unittest.TestCase):
    ...
    def test_complex(self):
        ...
        create_r_1 = requests.post(consts.API_WAREHOUSE_PREFIX + "create", json = wh1, headers=self.headers)
        self.assertEqual(create_r_1.status_code, 201)
        print("create_r_1: ")
        ...
        fetch_1_json=[wh1]
        print("fetch_1: ")
        fetch_r = requests.get(consts.API_WAREHOUSE_PREFIX + "fetch", headers = self.headers)
        print("     " , fetch_r.json())
        ...
        print("update_1: ")
        wh1["name"] = "ShanhaiSakura"
        update_r = requests.put(consts.API_WAREHOUSE_PREFIX + "update", json = wh1, headers=self.headers)
        ...
        print("     ", fetch_r.json())
        self.assertEqual(fetch_r.status_code, 200)
        self.assertEqual(fetch_r.json(), [wh1])
        
        print("fetch_one_1: ")
        fetch_one_r = requests.get(consts.API_WAREHOUSE_PREFIX + "fetch", params = {"id": wh1["id"]["id"]}, headers = self.headers)
        print("     ", fetch_one_r.json())
        self.assertEqual(fetch_one_r.status_code, 200)
        self.assertEqual(fetch_r.json(), [wh1])
        
        print("delete_2: ")
        delete_r = requests.delete(consts.API_WAREHOUSE_PREFIX + "delete", params = {"id": wh1["id"]["id"]}, headers = self.headers)
        self.assertEqual(delete_r.status_code, 204)
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(TestWareHouseAPI)
